...
/usr/local/bin/docker container rm ui db
...
volumes:
  ...
  db_vol:
    driver: local
'|USER_ID=`id -u` USER_NAME=`id -n -u` /usr/local/bin/docker compose -f - pull
...
volumes:
  ...
  db_vol:
    driver: local
'|USER_ID=`id -u` USER_NAME=`id -n -u` /usr/local/bin/docker compose -f - up \
